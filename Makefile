...
COMPILER ?= gcc
# Use for debugging GCC builds with GDB and Ares
DEBUGGING ?= 0
# Enable debug features
ENABLE_DEBUG_FEATURES ?= 0
...
ifeq ($(ENABLE_DEBUG_FEATURES),1)
  CFLAGS += -DENABLE_DEBUG_FEATURES
  CPPFLAGS += -DENABLE_DEBUG_FEATURES
endif
...
ASSET_BIN_DIRS_CUSTOM := $(shell find assets_custom/* -type d)

ASSET_BIN_DIRS := $(shell find assets/* -type d -not -path "assets/xml*" -not -path "assets/text")
ASSET_FILES_XML := $(foreach dir,$(ASSET_BIN_DIRS),$(wildcard $(dir)/*.xml))
ASSET_FILES_BIN := $(foreach dir,$(ASSET_BIN_DIRS) $(ASSET_BIN_DIRS_CUSTOM),$(wildcard $(dir)/*.bin))
...
# source files
C_FILES       := $(filter-out %.inc.c,$(foreach dir,$(SRC_DIRS) $(ASSET_BIN_DIRS) $(ASSET_BIN_DIRS_CUSTOM),$(wildcard $(dir)/*.c)))
...
TEXTURE_FILES_PNG := $(foreach dir,$(ASSET_BIN_DIRS) $(ASSET_BIN_DIRS_CUSTOM),$(wildcard $(dir)/*.png))
TEXTURE_FILES_JPG := $(foreach dir,$(ASSET_BIN_DIRS) $(ASSET_BIN_DIRS_CUSTOM),$(wildcard $(dir)/*.jpg))
TEXTURE_FILES_OUT := $(foreach f,$(TEXTURE_FILES_PNG:.png=.inc.c),build/$f) \
					 $(foreach f,$(TEXTURE_FILES_JPG:.jpg=.jpg.inc.c),build/$f) \

# create build directories
$(shell mkdir -p build/baserom build/assets/text $(foreach dir,$(SRC_DIRS) $(UNDECOMPILED_DATA_DIRS) $(ASSET_BIN_DIRS) $(ASSET_BIN_DIRS_CUSTOM),build/$(dir)))
...
build/assets_custom/%.o: assets_custom/%.c
...
build/assets_custom/%.jpg.inc.c: assets_custom/%.jpg
	$(ZAPD) bren -eh -i $< -o $@
