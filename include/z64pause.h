...
typedef enum {
    PAUSED_STATE_OFF,
    PAUSED_STATE_WAIT_LETTERBOX,       // Request no letterboxing and wait for it.
    PAUSED_STATE_WAIT_BG_PRERENDER,    // Wait for the pause background prerender to be done.
    PAUSED_STATE_INIT,                 // Load data and initialize/setup various things.
    PAUSED_STATE_OPENING,              // Animate the pause menu coming together with rotations and other animations.
    PAUSED_STATE_INVENTORY,            // Pause menu ready for player inputs.
    PAUSED_STATE_FADE_OUT_INVENTORY,   // Fade out inventory
    PAUSED_STATE_FADE_IN_SAVE_PROMPT,  // Fade into save prompt
    PAUSED_STATE_SAVE_PROMPT,          // Save prompt in the pause menu
    PAUSED_STATE_INIT_GAMEOVER,        // Init game over
    PAUSED_STATE_GAMEOVER_SAVE_PROMPT, // Game over save prompt
    PAUSED_STATE_GAMEOVER_CONTINUE_PROMPT, // Game over continue prompt
    PAUSED_STATE_CLOSING,                  // Animate the pause menu closing
    PAUSED_STATE_RESUME_GAMEPLAY // Handles returning to normal gameplay once the pause menu is visually closed
} PauseStateNew;

#define IS_PAUSE_STATE_GAMEOVER(pauseCtx) \
    (((pauseCtx)->state == PAUSED_STATE_INIT_GAMEOVER) || ((pauseCtx)->state == PAUSED_STATE_GAMEOVER_SAVE_PROMPT) || ((pauseCtx)->state == PAUSED_STATE_GAMEOVER_CONTINUE_PROMPT))

#define IS_PAUSED(pauseCtx) \
    (((pauseCtx)->state != PAUSED_STATE_OFF) || ((pauseCtx)->debugState != 0))

#define NEXT_PAUSE_STATE(pauseCtx) \
    ((pauseCtx->state & 0xFFFF) + 1)
...
typedef enum {
    PAUSE_MENU_CLOSE_SFX,
    PAUSE_MENU_OPEN_SFX
} PauseMenuOpenCloseSfx;
...
typedef enum {
    PAUSE_EQUIP_ANIM_STATE_OFF,
    PAUSE_EQUIP_ANIM_STATE_ANIMATE,
    PAUSE_EQUIP_ANIM_STATE_DONE
} PauseEquipAnimState;

typedef enum {
    PAUSE_GAMEOVER_PROMPT_CONTINUE,
    PAUSE_GAMEOVER_PROMPT_QUIT,
} PauseGameoverPromptChoice;

#define PAUSE_CURSOR_MOVEDIR_UP (1 << 0)
#define PAUSE_CURSOR_MOVEDIR_RIGHT (1 << 1)
#define PAUSE_CURSOR_MOVEDIR_DOWN (1 << 2)
#define PAUSE_CURSOR_MOVEDIR_LEFT (1 << 3)

#define PAUSE_CURSOR_MOVEDIR_HORIZONTAL (PAUSE_CURSOR_MOVEDIR_LEFT | PAUSE_CURSOR_MOVEDIR_RIGHT)
#define PAUSE_CURSOR_MOVEDIR_VERTICAL (PAUSE_CURSOR_MOVEDIR_UP | PAUSE_CURSOR_MOVEDIR_DOWN)

#define PAUSE_CURSOR_MOVEDIR_UP_LEFT (PAUSE_CURSOR_MOVEDIR_UP | PAUSE_CURSOR_MOVEDIR_LEFT)
#define PAUSE_CURSOR_MOVEDIR_UP_RIGHT (PAUSE_CURSOR_MOVEDIR_UP | PAUSE_CURSOR_MOVEDIR_RIGHT)
#define PAUSE_CURSOR_MOVEDIR_DOWN_LEFT (PAUSE_CURSOR_MOVEDIR_DOWN | PAUSE_CURSOR_MOVEDIR_LEFT)
#define PAUSE_CURSOR_MOVEDIR_DOWN_RIGHT (PAUSE_CURSOR_MOVEDIR_DOWN | PAUSE_CURSOR_MOVEDIR_RIGHT)
...
typedef struct {
    ...
    /* 0x0262 */ s16 promptChoice; // save/continue choice
    ...
    /* NEWVAR */ u8 inputDisabled;
    /* NEWVAR */ u8* pauseMenuSegment;
    /* NEWVAR */ u16 foregroundAlpha;
    /* NEWVAR */ InventoryCursor inventoryCursor;
    /* NEWVAR */ SaveCursor saveCursor;
    /* NEWVAR */ f32 cursorGlowTimer;
    /* NEWVAR */ u8 equipTimer;
    /* NEWVAR */ f32 equipAnimTimer;
    /* NEWVAR */ u8 equipAnimState;
