...
void Environment_Update(PlayState* play, EnvironmentContext* envCtx, LightContext* lightCtx, PauseContext* pauseCtx,
    ...
    if (pauseCtx->state == PAUSED_STATE_OFF) {
        ...
        if ((pauseCtx->state == PAUSED_STATE_OFF) && (gameOverCtx->state == GAMEOVER_INACTIVE)) {
...
void Environment_AdjustLights2(PlayState* play, f32 intensity, f32 targetFogNear, f32 intensityThreshold, f32 intensityScale, f32 targetZFar) {
    f32 temp;
    s32 i;
    ...
    if (play->roomCtx.curRoom.behaviorType1 != ROOM_BEHAVIOR_TYPE1_5 && Play_CamIsNotFixed(play)) {
        ...
        temp = intensity - intensityThreshold;

        if (intensity < intensityThreshold) {
            temp = 0.0f;
        }

        play->envCtx.adjFogNear = (targetFogNear - play->envCtx.lightSettings.fogNear) * temp;
        play->envCtx.adjZFar = (targetZFar - play->envCtx.lightSettings.zFar) * temp;
        ...
        for (i = 0; i < 3; i++) {
            play->envCtx.adjAmbientColor[i] = -(s16)(play->envCtx.lightSettings.ambientColor[i] * intensity);
            play->envCtx.adjLight1Color[i] = -(s16)(play->envCtx.lightSettings.light1Color[i] * intensity);
        }
    }
}
