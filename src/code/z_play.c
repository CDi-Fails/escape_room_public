...
void Play_Init(GameState* thisx) {
    ...
    SystemArena_Display();
    GameState_Realloc(&this->state, 4096 * 1000);
    ...
    if ((gSaveContext.gameMode != GAMEMODE_TITLE_SCREEN) && (gSaveContext.gameMode != GAMEMODE_NORMAL || gSaveContext.save.cutsceneIndex >= 0xFFF0)) {
...
void Play_Update(PlayState* this) {
    ...
    if (FrameAdvance_Update(&this->frameAdvCtx, &input[1])) {
        ...
        if (this->sceneId == SCENE_ESCAPE_ROOM) {
            switch (this->roomCtx.curRoom.num) {
                // Garden
                case 5:
                case 6:
                case 7:
                ...
                // Clock tower and archives
                case 4:
                case 16:
                    func_800F5ACC(NA_BGM_INSIDE_DEKU_TREE);
                    break;
                
                default:
                    func_800F5ACC(NA_BGM_UNDERGROUND);
                    break;
            }
        }
