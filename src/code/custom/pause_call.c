...
extern void Pause_Update(PlayState* play);
extern void Pause_Draw(PlayState* play);
...
void KaleidoScopeCall_Init(PlayState* play) {
    ...
    sKaleidoScopeUpdateFunc = KaleidoManager_GetRamAddr(Pause_Update);
    sKaleidoScopeDrawFunc = KaleidoManager_GetRamAddr(Pause_Draw);

    LOG_ADDRESS("kaleido_scope_move", Pause_Update, "../z_kaleido_scope_call.c", 98);
    LOG_ADDRESS("kaleido_scope_move_func", sKaleidoScopeUpdateFunc, "../z_kaleido_scope_call.c", 99);
    LOG_ADDRESS("kaleido_scope_draw", Pause_Draw, "../z_kaleido_scope_call.c", 100);
    ...
    if (IS_PAUSED(pauseCtx)) {
        if (pauseCtx->state == PAUSED_STATE_WAIT_LETTERBOX) {
            if (Letterbox_GetSize() == 0) {
                ...
                pauseCtx->unk_1EC = 0;
                pauseCtx->state = NEXT_PAUSE_STATE(pauseCtx); // PAUSE_STATE_WAIT_BG_PRERENDER
            }
        } else if (pauseCtx->state == PAUSED_STATE_INIT_GAMEOVER) {
            // Start game over
            R_HREG_MODE = HREG_MODE_UCODE_DISAS;
            R_UCODE_DISAS_LOG_MODE = 3;
            R_PAUSE_BG_PRERENDER_STATE = PAUSE_BG_PRERENDER_SETUP;
            pauseCtx->mainState = PAUSE_MAIN_STATE_IDLE;
            pauseCtx->unk_1EC = 0;
            pauseCtx->state = PAUSED_STATE_WAIT_BG_PRERENDER;
        } else if (pauseCtx->state == PAUSED_STATE_WAIT_BG_PRERENDER) {
            osSyncPrintf("PR_KAREIDOSCOPE_MODE=%d\n", R_PAUSE_BG_PRERENDER_STATE);

            if (R_PAUSE_BG_PRERENDER_STATE >= PAUSE_BG_PRERENDER_READY) {
                pauseCtx->state = PAUSED_STATE_INIT;
            }
        } else if (pauseCtx->state != PAUSED_STATE_OFF) {
