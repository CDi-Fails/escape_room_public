...
#include "assets_custom/objects/object_gameplay_keep/gFocusIndicatorDL.h"
...
void func_8002C124(TargetContext* targetCtx, PlayState* play) {
    ...
    if ((actor != NULL) && !(actor->flags & ACTOR_FLAG_27) && !(actor->flags & ACTOR_FLAG_DRAW_FOCUS_INDICATOR)) {
    ...
    if ((actor != NULL) && (actor->flags & ACTOR_FLAG_DRAW_FOCUS_INDICATOR)) {
        POLY_XLU_DISP = Gfx_SetupDL(POLY_XLU_DISP, SETUPDL_7);

        Matrix_Translate(actor->focus.pos.x, actor->focus.pos.y + (actor->targetArrowOffset * actor->scale.y) + 17.0f,
                         actor->focus.pos.z, MTXMODE_NEW);
        Matrix_RotateY(BINANG_TO_RAD((u16)(play->gameplayFrames * 3000)), MTXMODE_APPLY);
        Matrix_Scale((iREG(28) + 100) / 1000.0f, (iREG(28) + 80) / 1000.0f, (iREG(28) + 60) / 1000.0f, MTXMODE_APPLY);

        gDPSetPrimColor(POLY_XLU_DISP++, 0, 0, 215, 30, 30, 255);
        gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(play->state.gfxCtx, "../z_actor.c", 2153),
                  G_MTX_MODELVIEW | G_MTX_LOAD);
        gSPDisplayList(POLY_XLU_DISP++, gFocusIndicatorDL);
    }
...
int Actor_PlayerIsAimingFpsItem(Player* player) {
    return player->stateFlags1 & PLAYER_STATE1_AIMING_FPS_ITEM;
}

int Actor_PlayerIsAimingReadyFpsItem(Player* player) {
...
int func_8002DEEC(Player* player) {
    return (player->stateFlags1 & (PLAYER_STATE1_IN_DEATH_CUTSCENE | PLAYER_STATE1_IN_CUTSCENE)) || (player->csAction != PLAYER_CSACTION_NONE);
}
...
void Actor_MoveDynaPolyActor(DynaPolyActor* dynaActor, f32 moveDist, s16 moveAngle) {
    dynaActor->unk_150 += moveDist;
    dynaActor->unk_158 = moveAngle;
}
...
void Actor_UpdateAll(PlayState* play, ActorContext* actorCtx) {
    ...
    if ((actor != NULL) && (actor->update == NULL)) {
        actor = NULL;
        Player_ClearTargetActor(player);
    }
...
s32 Actor_IsTargeted(PlayState* play, Actor* actor) {
    ...
    if ((player->stateFlags1 & PLAYER_STATE1_Z_TARGETING_BATTLE) && actor->isTargeted) {
        return true;
    } else {
        return false;
    }
...
void Actor_DrawGreenCube(Actor* actor, PlayState* play) {
    ...
    Matrix_Translate(actor->world.pos.x, actor->world.pos.y + 10.0f, actor->world.pos.z, MTXMODE_NEW);

    Matrix_RotateY(actor->shape.rot.y * (M_PI / 32768), MTXMODE_APPLY);
    Matrix_RotateX(actor->shape.rot.x * (M_PI / 32768), MTXMODE_APPLY);
    Matrix_RotateZ(actor->shape.rot.z * (M_PI / 32768), MTXMODE_APPLY);
