...
#include "assets_custom/objects/object_link_boy/object_link_boy.h"
#include "assets_custom/objects/object_link_boy/escape_link_skel.h"
...
u8 sActionModelGroups[PLAYER_IA_MAX] = {
    ...
    PLAYER_MODELGROUP_DEFAULT,          // PLAYER_IA_WIND_CLOAK
};
...
Gfx* sPlayerWaistDLs[] = {
    gLinkAdultSkel_bone001_gLinkAdultWaistLimb_mesh_layer_Opaque,
...
Gfx* sPlayerRightHandHookshotDLs[] = {
    ...
    gLinkChildRightHandNearDL,
    gLinkAdultRightHandHoldingHookshotFarDL, // The 'far' display list exists but is not used
    gLinkChildRightHandNearDL,
};
...
Gfx* sFirstPersonRightShoulderDLs[] = {
    gLinkAdultSkel_bone016_gLinkAdultRightShoulderLimb_mesh_layer_Opaque,
...
void Player_SetBootData(PlayState* play, Player* this) {
    ...
    if (currentBoots == PLAYER_BOOTS_KOKIRI) {
        ...
        REG(27) = 500;
        REG(48) = 100;
    } else if (currentBoots == PLAYER_BOOTS_ZORA && this->stateFlags3 & PLAYER_STATE3_ZORA_SWIMMING) {
        currentBoots = PLAYER_BOOTS_ZORA_SWIMMING;
        REG(27) = 500;
        REG(48) = 100;
    } 
...
int Player_InBlockingCsMode(PlayState* play, Player* this) {
    return (this->stateFlags1 & (PLAYER_STATE1_IN_DEATH_CUTSCENE | PLAYER_STATE1_IN_CUTSCENE)) || (this->csAction != PLAYER_CSACTION_NONE) ||
...
s32 Player_CheckBattleTargeting(Player* this) {
    return (this->stateFlags1 & PLAYER_STATE1_Z_TARGETING_BATTLE);
}
...
void Player_SetItemActionFromHeldItem(Player* this) {
...
void Player_SetEquipmentData(PlayState* play, Player* this) {
    if (this->csAction != PLAYER_CSACTION_START_SWORD_KNOCKED_FROM_HAND) {
...
void Player_ClearTargetActor(Player* this) {
    this->targetActor = NULL;
    this->stateFlags2 &= ~PLAYER_STATE2_USING_SWITCH_Z_TARGETING;
}

void Player_ClearTargetAndSetMidairStatus(Player* this) {
    if ((this->actor.bgCheckFlags & BGCHECKFLAG_GROUND) ||
        ...
        this->stateFlags1 &= ~(PLAYER_STATE1_UNUSED_Z_TARGETING_FLAG | PLAYER_STATE1_Z_LOCK_ON | PLAYER_STATE1_Z_PARALLEL | PLAYER_STATE1_JUMPING |
                               PLAYER_STATE1_FREEFALLING | PLAYER_STATE1_TARGET_ACTOR_LOST);
...
void Player_SetTarget(PlayState* play, Actor* actor) {
    Player* this = GET_PLAYER(play);
        ...
        if ((triggerEntry->flag != 0) && !(gSaveContext.envHazardTextTriggerFlags & triggerEntry->flag) &&
            (((envHazard == (PLAYER_ENV_HAZARD_HOTROOM - 1)) && (this->currentTunic != PLAYER_TUNIC_GORON)) ||
             (((envHazard == (PLAYER_ENV_HAZARD_UNDERWATER_FLOOR - 1)) ||
              ...
              (this->currentBoots == PLAYER_BOOTS_IRON) && (this->currentBoots != PLAYER_BOOTS_ZORA)))) {
...
#ifndef AVOID_UB
void* sEyeTextures[] = {
    gLinkAdultSkel_eyes_open_ci8_ci8,      gLinkAdultSkel_eyes_half_ci8_ci8,  gLinkAdultSkel_eyes_closed_ci8_ci8, gLinkAdultSkel_eyes_roll_left_ci8_ci8,
    gLinkAdultSkel_eyes_roll_right_ci8_ci8, gLinkAdultSkel_eyes_shock_ci8_ci8, gLinkAdultSkel_eyes_unk_1_ci8_ci8,    gLinkAdultSkel_eyes_unk_2_ci8_ci8,
};

void* sMouthTextures[] = {
    gLinkAdultSkel_mouth_1_ci8_ci8,
    gLinkAdultSkel_mouth_2_ci8_ci8,
    gLinkAdultSkel_mouth_3_ci8_ci8,
    gLinkAdultSkel_mouth_4_ci8_ci8,
};
#else
// Defining `AVOID_UB` will use a 2D array instead and properly use the child link pointers to allow for shifting.
void* sEyeTextures[][8] = {
    { gLinkAdultSkel_eyes_open_ci8_ci8, gLinkAdultSkel_eyes_half_ci8_ci8, gLinkAdultSkel_eyes_closed_ci8_ci8,
      gLinkAdultSkel_eyes_roll_left_ci8_ci8, gLinkAdultSkel_eyes_roll_right_ci8_ci8, gLinkAdultSkel_eyes_shock_ci8_ci8,
      gLinkAdultSkel_eyes_unk_1_ci8_ci8, gLinkAdultSkel_eyes_unk_2_ci8_ci8 },
...
void* sMouthTextures[][4] = {
    { gLinkAdultSkel_mouth_1_ci8_ci8, gLinkAdultSkel_mouth_2_ci8_ci8, gLinkAdultSkel_mouth_3_ci8_ci8,
      gLinkAdultSkel_mouth_4_ci8_ci8 },
...
void Player_DrawImpl(PlayState* play, void** skeleton, Vec3s* jointTable, s32 dListCount, s32 lod, s32 tunic, s32 boots,
    ...
    if (GET_PLAYER(play)->currentTunic == PLAYER_TUNIC_GORON) {
    ...
    SkelAnime_DrawFlexOpa(play, skeleton, jointTable, dListCount, overrideLimbDraw, postLimbDraw, data);
    ...
    //         if (boots != PLAYER_BOOTS_KOKIRI && boots < PLAYER_BOOTS_ZORA) {
...
void func_8008F87C(PlayState* play, Player* this, SkelAnime* skelAnime, Vec3f* pos, Vec3s* rot, s32 thighLimbIndex,
    ...
    if ((this->actor.scale.y >= 0.0f) && !(this->stateFlags1 & PLAYER_STATE1_IN_DEATH_CUTSCENE) &&
        ...
        sp74 = D_80126068[(void)0, gSaveContext.save.linkAge] - this->shapeOffsetY;
        ...
        if (sp98.y < sp80) {
            ...
            if ((temp3 >= FLOOR_TYPE_HURT_FLOOR) && (temp3 <= FLOOR_TYPE_FIRE_HURT_FLOOR) && !func_80042108(&play->colCtx, sp88, sp84)) {
                footprintPos.y = sp80;
...
s32 Player_OverrideLimbDrawGameplayCommon(PlayState* play, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot,
    ...
    if (limbIndex == PLAYER_LIMB_ROOT) {
        ...
        if (this->stateFlags3 & PLAYER_STATE3_ZORA_SWIMMING) {
            ...
            Matrix_RotateZ(BINANG_TO_RAD(this->zoraSwimRoll), MTXMODE_APPLY);
            Matrix_RotateZYX(rot->x, rot->y, rot->z, MTXMODE_APPLY);
            pos->x = pos->y = pos->z = 0.0f;
            rot->x = rot->y = rot->z = 0;
        } else if (this->shapePitchOffset != 0) {
            Matrix_Translate(pos->x, ((Math_CosS(this->shapePitchOffset) - 1.0f) * 200.0f) + pos->y, pos->z, MTXMODE_APPLY);
            Matrix_RotateX(BINANG_TO_RAD(this->shapePitchOffset), MTXMODE_APPLY);
            Matrix_RotateZYX(rot->x, rot->y, rot->z, MTXMODE_APPLY);
            pos->x = pos->y = pos->z = 0.0f;
    ...
    if (!Player_OverrideLimbDrawGameplayCommon(play, limbIndex, dList, pos, rot, thisx)) {
        if (this->attentionMode != 2) {
            ...
            *dList = Player_HoldsHookshot(this)
                         ? NULL
                         : sFirstPersonRightShoulderDLs[(void)0, gSaveContext.save.linkAge];
...
void Player_PostLimbDrawGameplay(PlayState* play, s32 limbIndex, Gfx** dList, Vec3s* rot, void* thisx) {
    ...
    if (this->itemAction == PLAYER_IA_BOOSTERS) {
        ...
        switch (limbIndex) {
            case PLAYER_LIMB_L_FOREARM:
                gSPDisplayList(POLY_OPA_DISP++, gLinkAdultLeftBoosterDL);
                break;

            case PLAYER_LIMB_R_FOREARM:
                gSPDisplayList(POLY_OPA_DISP++, gLinkAdultRightBoosterDL);
                break;
    ...
    if (this->stateFlags3 & PLAYER_STATE3_ZORA_SWIMMING) {
        if (limbIndex == PLAYER_LIMB_L_HAND || limbIndex == PLAYER_LIMB_R_HAND) {
            Vec3f blurEndPos = { -250.0f, 4500.0f, 200.0f };
            Vec3f blurEndPosFinal;
            u8 index = (limbIndex == PLAYER_LIMB_L_HAND ? 0 : 1);

            if (limbIndex == (PLAYER_LIMB_R_HAND)) {
                blurEndPos.z *= -1.0f;
            }

            Matrix_MultVec3f(&blurEndPos, &blurEndPosFinal);

            EffectBlure_AddVertex(Effect_GetByIndex(this->zoraSwimEffectIndex[index]), sCurBodyPartPos, &blurEndPosFinal);
        }
    }
    ...
    if (limbIndex == PLAYER_LIMB_L_HAND) {
        ...
        if (this->rightHandType == PLAYER_MODELTYPE_RH_FF) {
            ...
            if ((this->stateFlags1 & PLAYER_STATE1_READY_TO_SHOOT) && (this->unk_860 >= 0) && (this->fpsItemTimer <= 10)) {
                Vec3f sp90;
                f32 distXYZ;
