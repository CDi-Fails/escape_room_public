...
void EffectSsKakera_Draw(PlayState* play, u32 index, EffectSs* this) {
    ...
    if (this->rObjId != KAKERA_OBJECT_DEFAULT) {
        if ((((this->rReg4 >> 7) & 1) << 7) == 0x80 || this->rObjId == OBJECT_GIANT_PLANT) {
    ...
    if ((((this->rReg4 >> 7) & 1) << 7) == 0x80 || this->rObjId == OBJECT_GIANT_PLANT) {
        ...
        if (this->rObjId == OBJECT_GIANT_PLANT) {
            u8 alpha = 255;

            if (this->life < 15) {
                alpha = (u8)(255 * ((f32)this->life / 15.0f));
            }
...
s32 func_809AA0EC(EffectSs* this) {
    ...
    if (this->rObjId != OBJECT_GIANT_PLANT && dist > 1000.0f) {
        return 0;
    }
    ...
    if (this->rReg8 == 0) {
        if ((((this->rReg4 >> 4) & 1) * 0x10) == 0x10) {
            if (this->pos.y <= (player->actor.floorHeight - ((this->rReg4 >> 2) & 3))) {
                ...
                // Hacky way of keeping track of if sound is played without changing effect struct
                if (this->rObjId == OBJECT_GIANT_PLANT && this->rColorIdx == KAKERA_COLOR_NONE) {
                    SfxSource_PlaySfxAtFixedWorldPos(play, &this->pos, 40, NA_SE_EV_COFFIN_CAP_BOUND);
                    this->rColorIdx = KAKERA_COLOR_WHITE;
                }
            }
        } else {
