...
static MirRayDataEntry sMirRayData[] = {
    // Just place the MirRay actor where needed, copy the position data to here, then add +300 to the Y position for the first point
    { { -2736, 1398 + 300, -9 }, { -2736, 1398, -9 }, 50, 100, 1.0f, 0, 150, 0.94f, 255, 255, 255, 0x03 },
    { { -4275, -207 + 300, -2078 }, { -4275, -207, -2078 }, 50, 100, 1.0f, 0, 150, 0.94f, 255, 255, 255, 0x03 },
...
#define MIR_RAY_REFLECT_LENGTH 800.0f
...
void MirRay_SetupReflectionPolys(MirRay* this, PlayState* play, MirRayShieldReflection* reflection) {
    ...
    sp60[0] = -((*shieldMtx).xz * this->reflectRange) * this->reflectIntensity * MIR_RAY_REFLECT_LENGTH;
    sp60[1] = -((*shieldMtx).yz * this->reflectRange) * this->reflectIntensity * MIR_RAY_REFLECT_LENGTH;
    sp60[2] = -((*shieldMtx).zz * this->reflectRange) * this->reflectIntensity * MIR_RAY_REFLECT_LENGTH;
...
void MirRay_ReflectedBeam(MirRay* this, PlayState* play, MirRayShieldReflection* reflection) {
    ...
    spE8[0] = -(shieldMtx->xz * this->reflectRange) * this->reflectIntensity * MIR_RAY_REFLECT_LENGTH;
    spE8[1] = -(shieldMtx->yz * this->reflectRange) * this->reflectIntensity * MIR_RAY_REFLECT_LENGTH;
    spE8[2] = -(shieldMtx->zz * this->reflectRange) * this->reflectIntensity * MIR_RAY_REFLECT_LENGTH;
