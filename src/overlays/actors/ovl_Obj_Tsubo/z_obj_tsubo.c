...
void ObjTsubo_SpawnCollectible(ObjTsubo* this, PlayState* play) {
    ...
    if ((CUR_UPG_VALUE(UPG_BOMB_BAG) > 0) && (AMMO(ITEM_BOMB) < CUR_CAPACITY(UPG_BOMB_BAG)) && (Rand_ZeroOne() < 0.5f)) {
        Item_DropCollectible(play, &this->actor.world.pos, ITEM00_BOMBS_A);
    } else {
