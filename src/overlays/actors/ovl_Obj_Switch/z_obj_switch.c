...
static u8 sNumMultiSwitchOn = 0;
static u8 sLeaderMultiSwitchFound = 0;
...
void ObjSwitch_Init(Actor* thisx, PlayState* play) {
    ...
    if (type == OBJSWITCH_TYPE_FLOOR || type == OBJSWITCH_TYPE_FLOOR_RUSTY) {
        ...
        if (this->dyna.actor.world.rot.x != 0) {
            ...
            if (!sLeaderMultiSwitchFound) {
                this->isMultiSwitchLeader = true;
                sLeaderMultiSwitchFound = true;
            }
            this->dyna.actor.world.rot.x = this->dyna.actor.shape.rot.x = 0;
            // Number of multi-switches in the room, stored in home.rot.z now
...
void ObjSwitch_Update(Actor* thisx, PlayState* play) {
    ...
    switch (OBJSWITCH_TYPE(&this->dyna.actor)) {
        ...
        case OBJSWITCH_TYPE_FLOOR_RUSTY:
            ...
            if (!Flags_GetSwitch(play, OBJSWITCH_SWITCH_FLAG(&this->dyna.actor)) && OBJSWITCH_SUBTYPE(&this->dyna.actor) == OBJSWITCH_SUBTYPE_HOLD && this->isMultiSwitchLeader) {
                if (sNumMultiSwitchOn == this->dyna.actor.home.rot.z) {
                    Flags_SetSwitch(play, OBJSWITCH_SWITCH_FLAG(&this->dyna.actor));
                    OnePointCutscene_AttentionSetSfx(play, &this->dyna.actor, NA_SE_SY_CORRECT_CHIME);
                }
            }
