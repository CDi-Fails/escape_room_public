...
void DoorShutter_BackLocked(DoorShutter* this, PlayState* play);
...
s32 DoorShutter_SetupDoor(DoorShutter* this, PlayState* play) {
    ...
    if (doorType == SHUTTER_FRONT_CLEAR) {
        ...
        if (play->sceneId == SCENE_ESCAPE_ROOM) {
            this->barsClosedAmount = 1.0f;
            DoorShutter_SetupAction(this, DoorShutter_BackLocked);
        } else {
...
/**
 * Special door which is locked only on the backside
 */
void DoorShutter_BackLocked(DoorShutter* this, PlayState* play) {
    if (this->dyna.actor.yawTowardsPlayer > 0 && ABS(this->dyna.actor.yDistToPlayer) < 100.0f) {
...
void DoorShutter_SetupClosed(DoorShutter* this, PlayState* play) {
    ...
    if (DoorShutter_SetupDoor(this, play) && !(player->stateFlags1 & PLAYER_STATE1_HOLDING_ACTOR)) {
        ...
        Player_SetCsActionWithHaltedActors(play, NULL, PLAYER_CSACTION_TURN_AROUND_SURPRISED_SHORT);
    }
}
