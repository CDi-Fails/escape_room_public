...
void EnTrap_Init(Actor* thisx, PlayState* play) {
    ...
    if (thisx->params & SPIKETRAP_MODE_LINEAR) {
        thisx->speed = this->moveSpeedForwardBack.z = (this->upperParams & 0xF) * 2.5f;
...
void EnTrap_Update(Actor* thisx, PlayState* play) {
    ...
    if ((player->actor.world.pos.y < 570.0f || player->actor.world.pos.y > 750.0f)) {
        return;
    }
    ...
    if (thisx->colorFilterTimer == 0) {
        ...
        if ((thisx->xzDistToPlayer <= 40.0f) && (this->playerDmgTimer == 0) && (thisx->yDistToPlayer <= 20.0f) && (thisx->yDistToPlayer >= -20.0f)) {
