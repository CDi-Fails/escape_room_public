...
void BossSst_HeadSetupDeath(BossSst* this, PlayState* play) {
    ...
    Player_SetCsActionWithHaltedActors(play, &player->actor, PLAYER_CSACTION_WAIT);
    Cutscene_StartManual(play, &play->csCtx);
...
void BossSst_HeadFinish(BossSst* this, PlayState* play) {
    ...
    if (this->effectMode == BONGO_NULL) {
        if (this->timer < -170) {
            ...
            Play_ChangeCameraStatus(play, CAM_ID_MAIN, CAM_STAT_ACTIVE);
            Play_ClearCamera(play, sSubCamId);
            Player_SetCsActionWithHaltedActors(play, &GET_PLAYER(play)->actor, PLAYER_CSACTION_END);
            Cutscene_StopManual(play, &play->csCtx);
            Actor_Kill(&this->actor);
