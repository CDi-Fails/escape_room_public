...
void FileSelect_ConfigModeDraw(GameState* thisx) {
    ...
    ZREG(11) += ZREG(10);
    Environment_UpdateSkybox(SKYBOX_CUTSCENE_MAP, &this->envCtx, &this->skyboxCtx);
    gDPPipeSync(POLY_OPA_DISP++);
...
void FileSelect_Main(GameState* thisx) {
    ...
    if ((this->configMode <= CM_NAME_ENTRY_TO_MAIN) || (this->configMode >= CM_UNUSED_DELAY)) {
        ...
        gDPSetCombineLERP(POLY_OPA_DISP++, PRIMITIVE, ENVIRONMENT, TEXEL0, ENVIRONMENT, TEXEL0, 0, PRIMITIVE, 0,
                          PRIMITIVE, ENVIRONMENT, TEXEL0, ENVIRONMENT, TEXEL0, 0, PRIMITIVE, 0);
        gDPSetPrimColor(POLY_OPA_DISP++, 0, 0, 255, 217, 100, this->controlsAlpha);
        gDPSetEnvColor(POLY_OPA_DISP++, 0, 0, 0, 0);
...
void FileSelect_InitContext(GameState* thisx) {
    ...
    this->highlightColor[1] = 240;
    this->highlightColor[2] = 155;
    ...
    this->windowColor[0] = 200;
    this->windowColor[1] = 170;
    this->windowColor[2] = 128;
    ...
    gSaveContext.skyboxTime = CLOCK_TIME(16, 40);
    gSaveContext.save.dayTime = CLOCK_TIME(16, 40);

    Skybox_Init(&this->state, &this->skyboxCtx, SKYBOX_CUTSCENE_MAP);
